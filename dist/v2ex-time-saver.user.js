...
`;
  ...
  async function createV2exApp(runtime) {
    ...
    function getCommentByNumber(num) {
      return $$(".no").filter((it) => it.textContent?.includes(String(num)))[0]?.closest(".cell[id]");
    }
    function getLastCommentByAuthorBeforeNumber(authorName, num) {
      return $$(`a[href="/member/${authorName}"].dark`).map((it) => it.closest(".cell[id]")).filter((it) => Boolean(it)).filter((it) => {
