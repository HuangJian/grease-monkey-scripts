...
用户要求「界面显示 RSS 模块占用缓存数值」，因此除测试门槛外，存储检查器（`save-dialog.tsx`）现在为**每个**已存键显示 UTF-8 字节数（`data-action="key-size"`，如 `822.0 KB`）：

- 新增 `shared-utils.ts` 的 `utf8ByteLength`（不依赖 `TextEncoder`，纯函数逐码点计算）与 `formatByteSize`。用 UTF-8 字节而非 `String.length` 是必要的：中文摘要每字符 3 字节，按字符算会把 822KB 报成 ~270K。
- 虚拟的 `xueqiu-hot` 条目不计体积（它的字节已算在 `xueqiu-news` 下），避免重复计数。
- 于是「RSS 阅读 缓存」这一行会直接给出实测体积，日后若要调整 `SUMMARY_KEEP_COUNT` 或 `MAX_SUMMARY_CHARS`，先看这里的数字。
...
## 10. 上线前浏览器验证清单（自动化覆盖不到的部分）

R1–R5 已全部实现并通过 `bun run check`（1644 例），但以下行为只能在真实浏览器里确认。安装 `dist/prism.debug.js`（保留 `console.debug`，便于按 `[gm-rss]` / `[gm-dashboard]` 过滤），打开 v2ex 任一页面：

| # | 场景 | 期望 |
| --- | --- | --- |
| 1 | 面板 tab 列表 | `browse` 组内出现「RSS 阅读」，位于雪球热帖之后、杂项之前 |
| 2 | 空态 | 首次打开显示「尚未添加订阅源，请通过 ⚙ 添加或导入 OPML」 |
| 3 | 添加订阅源 | ⚙ 里填 URL → 保存 → 卡片出现条目；TTL 123 分钟内不重复抓取 |
| 4 | 多源分组 | 多个源各自成块，标题可点开原站，块内显示「N 条未读」 |
| 5 | 折叠 | 某源未读 > 3 条时只显示 2 条 + 「…还有 N 条未读」，点击可展开/收起 |
| 6 | 已读语义 | 点行只展开摘要，未读数不变；点「打开原文」后条目消失、tab 徽标 −1 |
| 7 | 批量已读 / 隐藏 | 行尾「↑已读」把该行以上全部置已读；「×隐藏」移除该行 |
| 8 | 时间线视图 | 切到「时间线」后按时间倒序合并各源，行内带源名标签；刷新页面后仍是时间线 |
| 9 | 摘要裁剪 | 超过 30 条的旧条目展开显示「摘要已裁剪，点击打开原文」 |
| 10 | 单源失败隔离 | 故意加一个不存在的 URL → 只有该源显示「刷新失败：…」，其它源正常 |
| 11 | 全部源失败 | 只剩坏源时，面板显示错误并进入 1→2→5→10 分钟退避（不会疯刷） |
| 12 | 存储体积 | ⚙ 的存储检查器里「RSS 阅读 缓存」显示实际 KB 数（可与计划 R5.3 的 822KB 估算比对） |
| 13 | OPML | 导出后用其它阅读器导入验证；再把该文件导回，提示「新增 0 个，跳过 N 个」 |
| 14 | 站点范围 | RSS 只在这 5 个已 match 站点可用（`@match` 未扩大，O1 已知取舍）；后台刷新也只在停留这些站点时发生 |

若第 5/9/12 项的实际观感与预期不符（例如仍嫌缓存大、折叠阈值不合适），调整入口分别是 `FOLD_THRESHOLD`、`SUMMARY_KEEP_COUNT` / `MAX_SUMMARY_CHARS`，都集中在 `src/prism/rss/constants.ts`。
