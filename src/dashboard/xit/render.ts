...
export function getDueDateStatus(dateStr: string): 'overdue' | 'today' | 'tomorrow' | 'soon' | 'future' | 'invalid' {
...
export function renderXit(
  ...
  if (!wrapper) {
    ...
    container.insertAdjacentHTML(
      ...
      `<div class="gm-sp-xit">
        <div class="gm-sp-xit-controls">
          ...
          <div class="gm-sp-xit-search-row">
            <input type="text" class="gm-sp-xit-search" placeholder="🔍 搜索条目和标签..." />
          </div>
          <div class="gm-sp-xit-tags" style="display: none;"></div>
        </div>
    ...
    // Wire main controls
    const searchInput = wrapper.querySelector('.gm-sp-xit-search') as HTMLInputElement
    searchInput.addEventListener('input', () => {
      const state = getFilterState(wrapper)
      state.search = searchInput.value.trim()
      renderListAndTags(wrapper, lines)
    })

    const tabs = wrapper.querySelectorAll<HTMLButtonElement>('.gm-sp-xit-tab')
    tabs.forEach((tab) => {
...
function renderListAndTags(wrapper: HTMLElement, lines: XitLine[]): void {
  const state = getFilterState(wrapper)
  const listEl = wrapper.querySelector('.gm-sp-xit-list') as HTMLElement
  const tagsEl = wrapper.querySelector('.gm-sp-xit-tags') as HTMLElement
  ...
  if (isFiltering) {
    displayLines = lines.filter((line) => {
      ...
      if (state.status === 'open') {
        ...
        if (line.status === 'checked' || line.status === 'obsolete' || !isNearOrOverdue(line.dueDate)) {
          return false
        }
      }
  ...
  const listHtml = displayLines
    .map((line) => {
      ...
      // Highlight tags
      descHtml = descHtml.replace(/(#[\w\d\u4e00-\u9fa5_-]+(?:=(?:[^\s#]+|"[^"]*"|'[^']*'))?)/gi, (match) => {
        return `<span class="gm-sp-xit-tag">${match}</span>`
      })

      // Highlight due dates and show warnings
      descHtml = descHtml.replace(/(->\s*(\d{4}(?:-\d{2}-\d{2}|-\d{2}|-Q[1-4]|-W\d{1,2})?))/g, (match, p1, dateStr) => {
        const status = getDueDateStatus(dateStr)
        let alertText = ''
      ...
      const prioHtml = line.priority > 0
